...
from app.services.downloader_utils import build_search_queries
from app.services.downloader_mapper import get_mapped_candidates, resolve_via_mapped_urls
...
async def resolve_stream_with_anime_downloader(anilist_id: int, ep: int, type_: str, provider_override: Optional[str] = None):
    ...
    try:
        ...
        return _search_providers(data, ep, title, is_dub, provider_override)
...
async def _try_mapped_urls(anilist_id: int, ep: int, title: str, is_dub: bool, provider_override: Optional[str]) -> Optional[Dict[str, Any]]:
    candidates = await get_mapped_candidates(anilist_id, is_dub, provider_override)
    result = await resolve_via_mapped_urls(candidates, ep, title)
    if result:
        logger.info(f"Resolved direct stream via mapped {result['provider']} for id={anilist_id} ep={ep}")
    return result


def _search_providers(data: Dict[str, Any], ep: int, title: str, is_dub: bool, provider_override: Optional[str]) -> Optional[Dict[str, Any]]:
    queries = build_search_queries(data, is_dub)
    providers = [provider_override] if provider_override else ['gogoanime', '9anime', 'animepahe', 'twistmoe']
    
    for provider in providers:
        result = _search_single_provider(provider, queries, ep, title)
        if result:
            return result
    return None


def _search_single_provider(provider: str, queries: list, ep: int, title: str) -> Optional[Dict[str, Any]]:
    try:
        cls = get_anime_class(provider)
        search_results = _execute_queries(cls, queries)
        if not search_results:
            return None

        chosen_stub = _select_best_stub(search_results, ep)
...
def _select_best_stub(search_results: list, ep: int) -> Any:
    chosen_stub = None
    max_eps = -1
